...
public class BarGraphColor extends JPanel {
	...
	String graphLabel = "Product sales";
	String xLabel = "Products";
	String yLabel = "Total Price";
	...
	public BarGraphColor(int dataBound, int[] data) {

		this(800, 600, dataBound, data);

	}

	public BarGraphColor(int w, int h, int dataBound, int[] data) {
	...
	public void paint(Graphics g) {
		...
		try {
			...
			for (int i = 0; i < scaledData.length; i++) {
				...
				ar[i] = (Integer)list[i];
				p_id = ar[i];
				g2.drawString("Product id- " + p_id, x1, y2 + fontHeight);
